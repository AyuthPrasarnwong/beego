...
func GetLink(count int, currentPage int, totalPages int, site string, nextLink string, prevLink string) (link Link) {
	...
	if count > 0 && currentPage <= totalPages {
		if currentPage == 1 {
			link.Next = site + nextLink
		} else if currentPage == 2 {
			link.Previous = site + prevLink + "&page=1"
			link.Next = site + nextLink
		} else if currentPage == totalPages {
			link.Previous = site + prevLink
		} else {
			link.Previous = site + prevLink
			link.Next = site + nextLink
		}
